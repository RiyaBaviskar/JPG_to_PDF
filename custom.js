...
// Function to initiate file download without converting images again
function downloadPDF() {
    // Check if there is a previously generated PDF data
    if (data.length === 0) {
        alert("No PDF data available to download.");
        return;
    }

    // Create a new PDF document with existing data
    const pdfDoc = PDFLib.PDFDocument.create();
    const pagePromises = data.map(async (item) => {
        const jpgUrl = item.list.result;
        const jpgImageBytes = await fetch(jpgUrl).then((res) => res.arrayBuffer());
        const jpgImage = await pdfDoc.embedJpg(jpgImageBytes);
    ...
    // Save the PDF document and initiate download
    Promise.all(pagePromises).then(async () => {
        const pdfBytes = await pdfDoc.save();
        download(pdfBytes, pdfName.slice(0, -4) + '_download.pdf', 'application/pdf');
    });
}